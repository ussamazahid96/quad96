...
from simulator.inv_pend import InvPend
from simulator.utils import *
from simulator.plot import quad_plotter
from simulator.controllers import AttitudeController, PositionController, PendulumController
...
class Drone:
    def __init__(self, drone_params, controller_params, args=None):
        ...
        # adding the inverted pendulum
        self.pend = InvPend()

        self.ph_command = np.array([0., 1, -1, deg2rad(0)])
        self.att_command = np.array([deg2rad(0), deg2rad(0), deg2rad(0), 0.])
        
        # controllers
        self.position_controller = PositionController(controller_params, False, dt=self.pos_dt)      
        self.pend_controller     = PendulumController(controller_params, self.args.render_pid, dt=self.att_dt)
        ...
        if self.args.render_quad:
            ...
            self.plot = quad_plotter(self.drone_params.drone_body, self.R_linear, self.r.reshape(-1, 1), self.pend)
        
        # render in unity simulator
        if self.args.render_window:
            from simulator.network import ZMQPlug
            self.network_queue = None
            self.network_plug = ZMQPlug([self.X, self.pend.X], self.network_queue, port=self.args.port)
        
        # for adding disturbances via keyboard (it will just change the state of the quad)
        if self.args.keyboard_dis:
            from simulator.keyboard import KeyboardController
            self.keyboard_handle = KeyboardController(self.args, self.pend.X, scale = 0.1)
        ...
        if self.args.camera_dis:
            ...
            self.camera_handle = SimpleCameraController(self.pend.X, scale=0.1)
    ...
    # update the drone and the pend state
    def update_state(self, u):
        ...
        self.w      = self.X[9:12]
        self.t     += self.att_dt
        self.pend.update_state(acc_x=self.dX[3], acc_y=self.dX[4])
        return self.X
    ...
    def position_control(self, refSig):
        ...
        while(i < self.st*self.pos_steps and self.running):
            ...
            while(j < self.att_steps//self.pos_steps and self.running):
                rd, sd = self.position_controller(refSig[0:2], self.X, self.pend, self.t)
                self.pend_position = np.array([rd, sd, refSig[2]])
                
                roll, pitch, thrust = self.pend_controller(self.pend_position, self.X, self.pend, self.t)
