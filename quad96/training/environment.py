...
class drone_env:
    def __init__(self, drone_params, controller_params, args=None, target_position=np.array([0,0,-2.5], dtype=np.float32)):
        self.args = args
        self.drone = Drone(drone_params, controller_params, args=self.args)
        self.state_size = len(self.drone.get_state)+3
        self.action_size = 3
        self.env_bounds = 10
        ...
        if self.args.camera_pos:
            ...
            self.camera_handle = HandGestureCameraController(scale=0.1)
        ...
        if self.args.keyboard_pos and not self.args.crazyflie:
            from simulator.keyboard import KeyboardController
            self.keyboard_handle = KeyboardController(self.args, self.target_position)
    ...
    def reset(self):
        if self.args.crazyflie:
            self.drone.cf_handle.hover_thread.start()
            return self.reset_fixed()
        self.drone.attitude_controller.reset()
        self.drone.position_controller.reset()
        x     =  np.random.uniform(-0.5, 0.5)*(self.env_bounds/2)
        y     =  np.random.uniform(-0.5, 0.5)*(self.env_bounds/2)
        z     =  np.random.uniform(-0.1, -self.env_bounds/2 + 0.5) 
        roll  =  np.random.uniform(-1, 1)*self.angle_bound_init
        pitch =  np.random.uniform(-1, 1)*self.angle_bound_init
        ...
        state = np.concatenate((delta, self.drone.get_state[3:], [self.error_distance], \
                               [self.velocity_magnitude], [self.max_tilt]), axis=0) 
        return state

    # used when we have crazyflie attached via crazyradio
    def reset_fixed(self):
        self.drone.attitude_controller.reset()
        self.drone.position_controller.reset()
        x     =  0
        y     =  0
        z     =  self.target_position[2] + 0.5 
        roll  =  0
        pitch =  0
        yaw   =  self.yaw
        self.drone.set_position(np.array([x, y, z]))
    ...
    def assistant(self):
        roll, pitch, thrust = self.drone.position_controller(self.target_position[:3], self.drone.X, self.drone.t)
        # mapping [0-65535] thrust to [-29.5,29.5]
